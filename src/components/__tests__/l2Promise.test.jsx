// src/components/__tests__/l2Promise.test.jsx
//
// Unit tests for L2Promise component — candidate generation UI and flow
...
describe('L2Promise', () => {
  ...
  beforeEach(async () => {
    ...
    // Lock seed artifact (prerequisite for L2)
    await storage.saveCanonFacet('premise', projectId, {
      premise: 'A dark fantasy tale of betrayal',
      genre: 'Dark Fantasy',
      tone: 'dark',
    });
  ...
  it('renders advanced options collapsible section', async () => {
    render(<L2Promise storage={storage} projectId={projectId} />);
  ...
  it('displays locked seed artifact from L1', async () => {
    render(<L2Promise storage={storage} projectId={projectId} />);
  ...
  it('generates and displays promise candidates when button clicked', async () => {
    render(<L2Promise storage={storage} projectId={projectId} />);
    ...
    await waitFor(() => {
      expect(screen.getByText(/Cursed Prince seeking redemption/i)).toBeInTheDocument();
      expect(screen.getByText(/Rebel knight/i)).toBeInTheDocument();
      expect(screen.getByText(/Court mage/i)).toBeInTheDocument();
    });
  });
  ...
  it('enables lock button only when candidate is picked', async () => {
    ...
    const lockButton = screen.getByText(/LOCK & ADVANCE TO L3/i);
  ...
  it('locks stage with picked candidate artifact', async () => {
    const onLock = vi.fn();
    render(<L2Promise storage={storage} projectId={projectId} onLock={onLock} />);
    ...
    await waitFor(() => {
      expect(onLock).toHaveBeenCalledWith('promise', expect.objectContaining({
        protagonist: expect.stringContaining('Cursed Prince'),
        want: expect.stringContaining('Break the curse'),
        obstacle: expect.stringContaining('Ancient pact'),
      }));
    });
  });
});
